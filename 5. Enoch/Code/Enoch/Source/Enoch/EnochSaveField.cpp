...
void UEnochSaveField::VectorToTArray(const vector<vector<FreeLancerSaveData>>& VectorHalf)
{
...
vector<vector<FreeLancerSaveData>> UEnochSaveField::TarrayToVector()
{
	vector<vector<FreeLancerSaveData>> result;
	int idx = 0;
	for (int i = 0; i < HeightSize/2; ++i)
	{
		result.push_back(vector<FreeLancerSaveData>());
