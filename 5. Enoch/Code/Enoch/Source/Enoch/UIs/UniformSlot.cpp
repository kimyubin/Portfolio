...
void UUniformSlot::NativeOnMouseEnter(const FGeometry& InGeometry, const FPointerEvent& InMouseEvent)
{
	...
	MouseHover();
	Cast<UEnochGameInstance>(GetGameInstance())->MyPlayMenuUI->IsHoverUI = true;
}
void UUniformSlot::NativeOnMouseLeave(const FPointerEvent& InMouseEvent)
{
	Super::NativeOnMouseLeave(InMouseEvent);
	MouseUnHover();
	Cast<UEnochGameInstance>(GetGameInstance())->MyPlayMenuUI->IsHoverUI = false;	
}
...
void UUniformSlot::MouseHover()
{
	...
	if(MyFreeLancerTID==FreeLancerTemplateID::None)
		return;
	Cast<UEnochGameInstance>(GetGameInstance())->FlInfo->SetFLID(MyFreeLancerTID);
}
...
void UUniformSlot::SetMyFreeLancerInfo(const FreeLancerSaveData& FreelancerInfo)
{
...
void UUniformSlot::DropInvenSlot()
{
	...
	else if(IsSelected && !HaveFreelancer)
	{
		...
		case(PanelType::RecruitPanel):
			...
			if(CommanderIns->BuyFLRecruitToInvenWrap(SelectedFreelancer.SlotNum, SlotNum))
			{
				EGameInstance->DeselectedFLUpdate();					
			}
			else
