...
EnochCommander::EnochCommander()
{
	...
	RecruitFreeLancerList.assign(MAX_RECRUITSLOT,FreeLancerSaveData());
	InvenFreeLancerList.assign(MAX_INVENSLOT,FreeLancerSaveData());
	CombinePossibleList.assign(MAX_RECRUITSLOT,FreeLancerCombineData());
	
	
	FieldFreeLancerList = vector<vector<FreeLancerSaveData>>(EnochFieldData::GetHeight(),vector<FreeLancerSaveData>(EnochFieldData::GetWidth(),FreeLancerSaveData()));
...
FailReason EnochCommander::BuyFLRecruitToInven(const int &RecruitListIdx, const int &InvenListIdx)
{
	//일단 구매할때 5골드라고 할꺼
	if(gold<5)
		return FailReason::NotEnoughGolds;
	//인벤 슬롯에 용병이 이미 있음
	if(InvenFreeLancerList[InvenListIdx].FLTemplateID!=FreeLancerTemplateID::None)
		return FailReason::None;
	// 상점 슬롯에 용병이 없음
	if(RecruitFreeLancerList[RecruitListIdx].FLTemplateID==FreeLancerTemplateID::None)
		return FailReason::None;
		
	gold -= 5;
	
	InvenFreeLancerList[InvenListIdx] = RecruitFreeLancerList[RecruitListIdx];
	RecruitFreeLancerList[RecruitListIdx] = FreeLancerSaveData();
...
void EnochCommander::BringFLFieldToInven(const Vector2D& FieldCell, const int& InvenIdx)
{
	...
	FieldFreeLancerList[FieldCell.y][FieldCell.x] = FreeLancerSaveData();
...
{
	//용병 최대치 도달해서 스폰 실패
	if(!IsVaildFieldCount())
		return FailReason::FieldAlreadyFull;
	//인벤에 용병 없으면 대기
	if(InvenFreeLancerList[InvenIdx].FLTemplateID == FreeLancerTemplateID::None)
		return FailReason::None;
	FieldFreeLancerList[FieldCell.y][FieldCell.x] = InvenFreeLancerList[InvenIdx];
	InvenFreeLancerList[InvenIdx] = FreeLancerSaveData();
	fieldFreeLancerCount++;
	//합성 재탐색
	CombineRecruitList();
	return FailReason::Success;
}
FailReason EnochCommander::BuyDisposeFLRecruitToField(const Vector2D& FieldCell, const int& RecruitIdx)
{
	//용병 최대치 도달해서 구매 실패
	if(!IsVaildFieldCount())
		return FailReason::FieldAlreadyFull;
	...
	if(FieldFreeLancerList[FieldCell.y][FieldCell.x].FLTemplateID!=FreeLancerTemplateID::None)
		return FailReason::None;
	// 상점 슬롯에 용병이 없음
	if(RecruitFreeLancerList[RecruitIdx].FLTemplateID==FreeLancerTemplateID::None)
		return FailReason::None;
	...
	RecruitFreeLancerList[RecruitIdx] = FreeLancerSaveData();
...
FreeLancerCombineData EnochCommander::FindFLCanCombine(FreeLancerSaveData FindFL)
{
...
bool EnochCommander::CombineDelFL(const FreeLancerCombineData& CombineIdx)
{
	...
	{		
		...
		{
			FieldFreeLancerList[it->y][it->x] = FreeLancerSaveData();
			--fieldFreeLancerCount;
		}
	}
	if(!CombineIdx.InvenList.empty())
	{
		...
		{
			InvenFreeLancerList[*it] = FreeLancerSaveData();
		}
	}
	return true;
}
...
bool EnochCommander::IsVaildFieldCount()
{
	//현재 규칙.
