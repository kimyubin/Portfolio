 ...
 */
class ENOCH_API EnochFiniteStateMachine
{
public:
	EnochFiniteStateMachine() = delete;
	
	static void Decide(class EnochFreeLancerData *freelancer);
...
private:
	...
	*/
	static Vector2D GetClosest(class EnochFreeLancerData* freeLancer, int owner, std::function<bool(EnochFreeLancerData *)>constraint = [](EnochFreeLancerData*) {return true; });
	static inline int CalculateDistance(Vector2D src, Vector2D dst);
	static Vector2D FindPath(class EnochFreeLancerData* freeLancer, Vector2D dst);
};