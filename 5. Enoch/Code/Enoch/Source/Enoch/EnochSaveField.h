...
public:
	...
	UPROPERTY()
	TArray<FreeLancerTemplateID> FLTemplateIDArray;
	UPROPERTY()
	TArray<uint8> FLLvArray;
	
	void VectorToTArray(const vector<vector<FreeLancerSaveData>>& VectorHalf);
	vector<vector<FreeLancerSaveData>> TarrayToVector();
	/**세이브할 데이터 셋팅
