...
TMap<uint8, TSharedPtr<TMap<FString, TSharedPtr<UPaperFlipbook>>>> AFlipbookLoader::FLFlipbookMap;
TMap<uint8, TSharedPtr<TMap<FString, TSharedPtr<UPaperFlipbook>>>> AFlipbookLoader::PJFlipbookMap;
...
#define REGISTER_FLIP(Type, ID) \
	{ \
		...
		ADD_FLIP(Type, Dead); \
		FLFlipbookMap.Add(ID, book); \
	}

#define REGISTER_FLIP_PJ(Type, ID) \
	{ \
		auto book = MakeShared<TMap<FString, TSharedPtr<UPaperFlipbook>>>(); \
		ADD_FLIP(Type, Idle); \
		ADD_FLIP(Type, Attack); \
		PJFlipbookMap.Add(ID, book); \
	}

	/////////////////////////////////////////////// 
	FLFlipbookMap = TMap<uint8, TSharedPtr<TMap<FString, TSharedPtr<UPaperFlipbook>>>>();
	PJFlipbookMap = TMap<uint8, TSharedPtr<TMap<FString, TSharedPtr<UPaperFlipbook>>>>();
	REGISTER_FLIP(Melee1, 1);
	REGISTER_FLIP(Melee2, 2);
	REGISTER_FLIP(Ranged1, 3);
	REGISTER_FLIP(Ranged2, 4);
	REGISTER_FLIP(Assassin1, 5);

	REGISTER_FLIP_PJ(RedDot, 101 );
...
TSharedPtr<TMap<FString, TSharedPtr<UPaperFlipbook>>> AFlipbookLoader::GetFlipbook(uint8 type)
{	
	if(type > 100) 
		return PJFlipbookMap.Contains(type) ? PJFlipbookMap[type] : nullptr;
	auto modelID =FreeLancerTemplate::GetFreeLancerModelID(type);	
	return FLFlipbookMap.Contains(modelID) ? FLFlipbookMap[modelID] : nullptr;
}
/*
