...
public:	
	...
	static class AEnochFreeLancer* SpawnFreeLancerByTID(uint8 type, uint8 level, bool isEnemy=false);	//풀에서 새 프리랜서를 하나 꺼냄
...
private :
	...
	static class AEnochFreeLancer* _SpawnFreeLancerActor(int32 SerialNumber);	//이미 데이터가 있는 프리랜서의 액터를 생성함
	
	static class AEnochProjectile* _SpawnProjectileActor(int32 SerialNumber);	//이미 데이터가 있는 투사체의 액터를 생성함
